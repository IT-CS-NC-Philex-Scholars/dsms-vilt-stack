...
use Filament\Widgets\ChartWidget; // <-- Import this trait
use Illuminate\Support\Facades\DB;
use Filament\Widgets\Concerns\InteractsWithPageFilters;

final class ScholarYearLevelDistributionChart extends ChartWidget
{
    ...
    protected function getData(): array
    {
        ...
        foreach ($types as $type) {
            ...
            foreach ($yearLevels as $level) {
                $count = $data->firstWhere(fn ($item): bool => $item->type === $type && $item->year_level === $level)?->count ?? 0;
                $typeData[] = $count;
            }
        ...
        $labels = $yearLevels->map(function (string $level) use ($data): string {
            ...
            if ($predominantType === 'High_School') {
